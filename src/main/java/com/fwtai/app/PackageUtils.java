...
public final class PackageUtils {
    ...
    public static void createDirtories() throws Exception {
        final File dir = new File(Config.basePath);
        deleteAllFilesOfDir(dir);// 优先删除文件夹，然后创建
        //创建文件路径后，拷贝文件
        final String[] files = Config.files;
        for (int i = 0; i <files.length; i++){
            String filepath = Config.basePath+files[i];
            createDir(Config.basePath.replaceAll("\\\\","\\\\\\\\"), Config.targetPath,filepath);
        }
        final File introductionFile = new File(Config.updateFile);
        ...
        final OutputStreamWriter pw = new OutputStreamWriter(new FileOutputStream(introductionFile),"GBK");
        ...
        final String zipName= Config.zipBasePath;
     ...
     * @date: 2018/6/4  23:55
     **/
    public static boolean createDir(final String replaceFisrt,final String replaceSecond,String destDirName) throws Exception {
        final String tmpPath = destDirName;
        final File sourceFile = new File(tmpPath.replaceAll(replaceFisrt,""));
        final File file = new File(destDirName);
    ...
    private static void saveFiles(File sourceFile, String destDirName) throws Exception{
        //拷贝文件到更新文件夹中
        final List<String> list =getAllFile(sourceFile.getAbsolutePath(),false);
        FileChannel inChannel=null;
        FileChannel outChannel=null;
        for (int i = 0; i < list.size(); i++) {
            final File tmpFile = new File(list.get(i));
            final String destFile = destDirName+File.separator+tmpFile.getName();
   ...
   */
     ...
     * @date: 2018/6/4  23:56
    */
    private static void deleteAllFilesOfDir(final File path) {
        if (!path.exists())
            return;
        if (path.isFile()) {
            path.delete();
